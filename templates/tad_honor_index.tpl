      ...
      <div class="alert alert-info">
        <div class="pull-right float-right pull-end">
          <{if $smarty.session.tad_honor_adm or ($post_power and $uid==$honor_uid)}>
            <a href="javascript:delete_tad_honor_func(<{$honor_sn|default:''}>);" class="btn btn-sm btn-danger"><{$smarty.const._TAD_DEL}></a>
            <a href="<{$xoops_url}>/modules/tad_honor/index.php?op=tad_honor_form&honor_sn=<{$honor_sn|default:''}>" class="btn btn-sm btn-warning"><{$smarty.const._TAD_EDIT}></a>
            <a href="<{$xoops_url}>/modules/tad_honor/index.php?op=tad_honor_form" class="btn btn-sm btn-primary"><{$smarty.const._TAD_ADD}></a>
          <{/if}>
        </div>
<{*        <{$honor_unit|default:''}> <{$uid_name|default:''}> 於 <{$honor_date|default:''}> 發布，共有 <{$honor_counter|default:''}> 人次閱讀*}>
          <{$lang_viewsinfo|default:''}>
      </div>
    </div>
<{/if}>
